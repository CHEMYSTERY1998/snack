...
- 🏠 **房间系统**: 创建/加入房间，支持密码保护
- ⚡ **道具系统**: 加速、减速、穿墙、无敌、缩短对手、磁铁（道具占据2x2格子）
...
- **方向键** 或 **WASD**: 控制蛇的移动方向（每次移动只能转一次方向）
- **P 键**: 暂停/继续游戏
- 吃到食物增加长度和分数
- 撞墙或撞到自己会死亡
...
| 磁铁 | 🧲 | 吸引附近食物向蛇头移动 | 次数（每tick消耗一次） |

## 游戏机制

- **出生保护**: 新出生/复活的蛇有1秒无敌时间
- **碰撞规则**: A碰到B的身体，A死亡，B不受影响
- **复活机制**: 死亡后等待3秒复活，所有道具属性清零，分数和累计长度保留
- **穿墙使用**: 穿墙时自动消耗一次次数
- **无敌使用**: 受到致命伤害时自动消耗一次次数
- **暂停功能**: 暂停后停止移动和复活，可以观战
- **属性显示**: 右侧面板显示当前玩家的道具属性
- **积分榜**: 显示累计分数和累计长度（跨生命累计）
